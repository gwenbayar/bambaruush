...
- **Earn currency for *trying*, not for *succeeding*.** A child receives Odon for *participating in* a round (engaging, attempting), not as a prize gated on correct answers. This protects healthy **intrinsic motivation** — the desire to learn for its own sake — and reinforces the no-failure-states rule: no child is left empty-handed for getting something wrong.
- **Keep intrinsic motivation in mind everywhere.** Rewards should support the joy of learning, never replace it. Favor effort/process praise ("you worked that out") over trait praise ("you're so smart"); let the sky-star — Od's *own* visible growth — be the accomplishment, not a treat handed over for approval.
